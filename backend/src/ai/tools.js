 ...
 * 
 * 5 tools for the agentic system:
 *   1. createAccount — user registration
 *   2. registerPet — pet registration
 *   3. findAvailableVets — vet lookup
 *   4. bookAppointment — appointment booking
 *   5. searchMedicalGuidelines — RAG knowledge retrieval
 */
...
export const registerPet = tool({
  description:
    ...
    'If the user mentions their pet or wants to book, ensure they have a pet registered first.',
  parameters: z.object({
...
export const findAvailableVets = tool({
  ...
  execute: async ({ limit }) => {
    ...
    if (error) {
      // Fallback: fetch vets from users table directly
...
export const bookAppointment = tool({
  ...
  execute: async ({ pet_id, vet_user_id, appointment_time, reason }) => {
    ...
    if (error) {
      if (error.message.includes('unique_vet_appointment')) {
        return { success: false, error: 'This vet already has an appointment at that time. Please choose a different time.' };
      }
      return { success: false, error: error.message };
    }
...
// ─────────────────────────────────────────────────
// Tool 5: searchMedicalGuidelines
// ─────────────────────────────────────────────────
export const searchMedicalGuidelines = tool({
  description:
    'Search the PetPulse veterinary knowledge base for medical information. ' +
    'Use this for ANY pet health question — symptoms, diseases, nutrition, preventive care, etc. ' +
    'Returns relevant knowledge chunks with source citations.',
  parameters: z.object({
  ...
  execute: async ({ query }) => {
    ...
    try {
      ...
      if (!results || results.length === 0) {
        return {
          success: true,
          message: 'No relevant medical information found in the knowledge base.',
          chunks: [],
          count: 0,
        };
      }
      ...
      return {
        success: false,
        error: 'Medical knowledge search is temporarily unavailable.',
        chunks: [],
      };
    }
  },
