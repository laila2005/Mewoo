/**
 * PetPulse — Email notifications for direct messages and connection requests.
 *
 ...
 * rapid back-and-forth, and one email per message would make PetPulse the thing
...
const fullName = (r) => `${r?.first_name || ''} ${r?.last_name || ''}`.trim() || 'A PetPulse member';
...
export async function emailOnNewMessage(senderId, receiverId, content) {
  try {
    ...
    await sendNotificationEmail(row.to_email, {
      subject: `${sender} sent you a message on PetPulse`,
      ...
      message:
        ...
        `<br/><br/>Open PetPulse to read it and reply.`,
...
export async function emailOnConnectionRequest(senderId, receiverId) {
  try {
    ...
    await sendNotificationEmail(row.to_email, {
      subject: `${sender} wants to connect with you on PetPulse`,
      heading: 'New connection request',
      message:
        `${row.to_first_name ? 'Hi ' + row.to_first_name + ', ' : ''}<strong>${sender}</strong> (${roleLabel}) ` +
        `would like to connect with you on PetPulse.<br/><br/>Accept the request to start chatting.`,
