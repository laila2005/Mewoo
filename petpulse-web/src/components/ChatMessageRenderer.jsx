...
// ─── Pet Card (shared by mating + adoption) ─────
const PetCard = ({ pet, accent, actionLabel, onAction }) => (
  <div className="rounded-xl overflow-hidden border bg-white" style={{ borderColor: accent.border, boxShadow: `0 4px 16px ${accent.shadow}` }}>
    <div className="h-24 bg-slate-100">
      {pet.avatar_url && <img src={pet.avatar_url} alt={pet.name} className="w-full h-full object-cover" />}
    </div>
    <div className="p-3">
      ...
      {actionLabel && (
        <button
          ...
          className="w-full mt-2 py-1.5 rounded-lg text-[10px] font-bold transition-all active:scale-95"
...
// ─── Mating Matches ─────────────────────────────
const MatingMatch = ({ data, onProposeMatch }) => (
  <div className="rounded-xl overflow-hidden border border-rose-100" style={{ boxShadow: '0 4px 16px rgba(244,63,94,0.08)' }}>
    <div className="bg-gradient-to-r from-rose-500 to-pink-600 px-4 py-2.5 flex items-center gap-2">
      ...
      <span className="text-white font-bold text-xs">Compatible Mating Partners ({data.count})</span>
    </div>
    <div className="bg-white p-3 grid grid-cols-2 gap-2.5">
      {(data.matches || []).map((pet, i) => (
        <PetCard
          key={pet.pet_id || i}
          pet={pet}
          accent={{ border: '#ffe4e6', shadow: 'rgba(244,63,94,0.10)', chipBg: '#fff1f2', chipText: '#e11d48' }}
          actionLabel="🐾 Propose Match"
          onAction={(p) => onProposeMatch?.(p)}
        />
      ))}
    </div>
  </div>
...
// ─── Adoption ───────────────────────────────────
const Adoption = ({ data, onNavigate }) => (
  <div className="rounded-xl overflow-hidden border border-emerald-100" style={{ boxShadow: '0 4px 16px rgba(16,185,129,0.08)' }}>
    <div className="bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-2.5 flex items-center gap-2">
      ...
      <span className="text-white font-bold text-xs">Pets Available for Adoption ({data.count})</span>
    </div>
    <div className="bg-white p-3 grid grid-cols-2 gap-2.5">
      {(data.pets || []).map((pet, i) => (
        <PetCard
          key={pet.pet_id || i}
          pet={pet}
          accent={{ border: '#d1fae5', shadow: 'rgba(16,185,129,0.10)', chipBg: '#ecfdf5', chipText: '#059669' }}
          actionLabel="View Adoption"
          onAction={() => onNavigate?.('/adoption')}
        />
      ))}
    </div>
  </div>
...
// ─── Provider List (vets / trainers by name) ────
const ProviderList = ({ data }) => (
  <div className="rounded-xl overflow-hidden border border-blue-100" style={{ boxShadow: '0 4px 16px rgba(59,130,246,0.08)' }}>
    <div className="bg-gradient-to-r from-blue-500 to-indigo-500 px-4 py-2.5 flex items-center gap-2">
      ...
      <span className="text-white font-bold text-xs">{data.role === 'trainer' ? 'Trainers' : 'Veterinarians'} ({data.count})</span>
    </div>
    <div className="bg-white divide-y divide-slate-100">
...
const Navigation = ({ data, onNavigate }) => (
  <button
    ...
    className="inline-flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition-all active:scale-95"
  >
 ...
 */
const ChatMessageRenderer = ({ blocks = [], onProposeMatch, onNavigate }) => {
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {blocks.map((block, index) => {
        ...
        return <Renderer key={index} data={block.data} onProposeMatch={onProposeMatch} onNavigate={onNavigate} />;
      })}
    </div>
  );
};
