...
const Marketplace = () => {
    ...
    const handleAddFromDetails = () => {
        ...
        setCart(prev => {
            ...
            if (existing) return prev.map(c => c.id === selectedProduct.id ? { ...c, qty: Math.min(selectedProduct.quantity !== undefined ? selectedProduct.quantity : 99, c.qty + detailQuantity) } : c);
            return [...prev, { ...selectedProduct, qty: detailQuantity }];
        });
        toast.success(`${selectedProduct.title.slice(0, 30)}... (${detailQuantity}) added to cart!`);
    ...
    return (
        <div className="bg-[#f7faf9] min-h-screen">
                    ...
                    {filtered.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {filtered.map(item => (
                                <div 
                                    key={item.id} 
                                    onClick={() => { setSelectedProduct(item); setDetailQuantity(1); }}
            ...
            {selectedProduct && (
                <div className="fixed inset-0 z-[250] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
                    <div className="bg-white rounded-3xl shadow-2xl max-w-3xl w-full overflow-hidden border border-slate-100 transform scale-100 transition-all duration-300 flex flex-col md:flex-row relative">
                        
                        {/* Close button */}
                        <button 
                            onClick={() => setSelectedProduct(null)}
                            className="absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center bg-white/80 hover:bg-slate-100 text-slate-500 hover:text-slate-800 rounded-full transition-colors shadow-sm border border-slate-100 backdrop-blur-md active:scale-95"
                        >
                            <span className="material-symbols-outlined text-[22px]">close</span>
                        </button>

                        {/* Left: Image & Badges */}
                        <div className="w-full md:w-1/2 relative bg-slate-50 flex items-center justify-center h-64 md:h-auto min-h-[300px]">
                            <img 
                            ...
                            />
                            {/* Category Badge */}
                            <span className="absolute top-4 left-4 py-1 px-3 bg-white/95 backdrop-blur-md text-blue-600 border border-slate-100 rounded-full text-xs font-bold shadow-sm uppercase tracking-wider">
                                {selectedProduct.category}
                            </span>
                            {/* Special Promo Badge */}
                            {selectedProduct.badge && (
                                <span className={`absolute top-12 left-4 px-3 py-1 rounded-full text-xs font-bold shadow-sm ${BADGE_COLORS[selectedProduct.badge] || 'bg-slate-600 text-white'}`}>
                                    {selectedProduct.badge}
                                </span>
                            )}
                        </div>
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    {selectedProduct.shop_name ? (
                                        <button 
                                            onClick={() => {
                                                navigate(`/marketplace?shop=${encodeURIComponent(selectedProduct.shop_name)}`);
                                                setSelectedProduct(null);
                                            }}
                                            className="text-xs font-bold text-emerald-600 hover:text-emerald-700 hover:underline text-left animate-pulse"
                                        >
                                            {selectedProduct.shop_name}
                                        </button>
                                    ) : (
                                        <span className="text-xs font-bold text-slate-400">PetPulse Official</span>
                                    )}
                                </div>

                                <h2 className="text-2xl font-extrabold text-slate-900 mb-3 leading-snug">{selectedProduct.title}</h2>
                                
                                {/* Ratings */}
                                <div className="flex items-center gap-3 mb-5 pb-5 border-b border-slate-100">
                                    <StarRating rating={selectedProduct.rating} />
                                    <span className="text-xs text-slate-400 font-medium">({selectedProduct.reviews} verified reviews)</span>
                                </div>

                                <p className="text-sm text-slate-600 mb-6 leading-relaxed line-clamp-4">{selectedProduct.description}</p>
                                    ...
                                    {selectedProduct.quantity !== undefined ? (
                                        selectedProduct.quantity > 0 ? (
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${
                                                selectedProduct.quantity > 3 ? 'bg-emerald-50 text-emerald-700 border border-emerald-250' : 'bg-amber-50 text-amber-700 border border-amber-250'
                                            }`}>
                                                <span className="w-1.5 h-1.5 rounded-full bg-current animate-pulse"></span>
                                                {selectedProduct.quantity} Items Available
                                            </span>
                                        ) : (
                                            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-700 border border-red-250 rounded-full text-xs font-bold">
                                                <span className="w-1.5 h-1.5 rounded-full bg-current"></span>
                                                Out of Stock
                                            </span>
                                        )
                                    ) : (
                                        <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-700 border border-blue-250 rounded-full text-xs font-bold">
                                            <span className="w-1.5 h-1.5 rounded-full bg-current animate-pulse"></span>
                                            Available in Stock
                                        </span>
                                    )}
                            ...
                            <div>
                                ...
                                <div className="flex items-center justify-between gap-4 mb-6 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                    <div className="flex flex-col">
                                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-0.5">Unit Price</span>
                                        <span className="font-extrabold text-blue-600 text-2xl">{selectedProduct.base_price.toLocaleString()} <span className="text-xs font-bold">EGP</span></span>
                                    </div>
                                    ...
                                    <div className="flex items-center bg-white rounded-xl border border-slate-200 p-1 shadow-sm">
                                        <button 
                                            ...
                                            className="w-8 h-8 rounded-lg hover:bg-slate-100 text-slate-600 font-bold text-lg flex items-center justify-center active:scale-90 transition-transform"
                                        >
                                            -
                                        </button>
                                        <span className="w-10 text-center font-bold text-slate-800 text-sm select-none">{detailQuantity}</span>
                                        <button 
                                            type="button"
                                            onClick={() => setDetailQuantity(prev => selectedProduct.quantity !== undefined ? Math.min(selectedProduct.quantity, prev + 1) : prev + 1)}
                                            className="w-8 h-8 rounded-lg hover:bg-slate-100 text-slate-600 font-bold text-lg flex items-center justify-center active:scale-90 transition-transform"
                                            disabled={selectedProduct.quantity !== undefined && selectedProduct.quantity <= detailQuantity}
                                        >
                                            +
                                        </button>
                                    </div>
                                ...
                                <div className="flex gap-3">
                                    <button 
                                        ...
                                        disabled={selectedProduct.quantity !== undefined && selectedProduct.quantity === 0}
                                        className="flex-1 py-4 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-400 text-white font-bold rounded-2xl shadow-lg hover:shadow-blue-500/25 hover:-translate-y-0.5 active:translate-y-0 active:scale-98 transition-all flex items-center justify-center gap-2"
                                    >
                                        <span className="material-symbols-outlined text-[20px]">add_shopping_cart</span>
                                        Add to Cart • {(selectedProduct.base_price * detailQuantity).toLocaleString()} EGP
                                    </button>
                                </div>
                            </div>
