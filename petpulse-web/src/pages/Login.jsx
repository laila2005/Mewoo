...
const Login = () => {
    ...
    const submitGoogleLogin = async (e) => {
        e.preventDefault();
        if (!googleEmail || !googleFirstName) {
            toast.error('Please enter your email and first name.');
            return;
        }
        setLoading(true);
        try {
            ...
            const res = await axios.post(`${API_BASE}/auth/google`, {
                ...
                last_name: googleLastName || 'Guest',
        ...
        <div className="flex min-h-screen bg-slate-50">
                ...
                <div className="w-full max-w-[420px] mx-auto">
                    ...
                    <div className="space-y-6">
                        <button 
                            ...
                            type="button"
                            disabled={loading}
                            className="w-full py-3 px-4 border border-slate-200 rounded-xl flex items-center justify-center gap-3 hover:bg-slate-50 transition-colors font-bold text-slate-700 text-sm shadow-sm active:scale-[0.98] disabled:opacity-75"
                        >
                        ...
                        <form onSubmit={handleLogin} className="space-y-5">
                            ...
                            <div className="space-y-1.5">
                                <div className="flex justify-between items-center px-1">
                                    ...
                                    <Link className="font-bold text-blue-600 text-[12px] hover:underline" to="/forgot-password">Forgot Password?</Link>
                                </div>
                                <div className="relative flex items-center">
            ...
            {showGoogleModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-[420px] w-full mx-4 overflow-hidden border border-slate-100 transform scale-100 transition-all duration-300">
                        {/* Google top colored bar */}
                        <div className="h-1.5 w-full bg-gradient-to-r from-red-500 via-amber-400 to-green-500 flex">
                            ...
                            <form onSubmit={submitGoogleLogin} className="space-y-4">
                                <div className="space-y-1">
                                    ...
                                    <input 
                                        ...
                                        className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-[#4285F4] focus:bg-white focus:ring-4 focus:ring-blue-100 rounded-xl transition-all outline-none text-sm font-medium"
                                    />
                                </div>
                                ...
                                <div className="flex items-center justify-between gap-4 pt-4">
                                    <button 
                                        ...
                                        className="px-4 py-2.5 text-sm font-bold text-slate-500 hover:bg-slate-100 rounded-xl transition-colors active:scale-95"
                                    >
                                        Cancel
                                    </button>
                                    <button 
                                        type="submit"
                                        disabled={loading}
                                        className="px-6 py-2.5 bg-[#4285F4] text-white text-sm font-bold rounded-xl hover:bg-[#3574de] active:scale-[0.98] transition-all shadow-md flex items-center gap-1.5 disabled:opacity-75"
                                    >
                                        {loading ? 'Connecting...' : 'Sign In'}
                                        {!loading && <span className="material-symbols-outlined text-[16px]">done</span>}
                                    </button>
                                </div>
                            </form>
                        </div>
