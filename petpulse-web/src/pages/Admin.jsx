...
const Admin = () => {
                                    ...
                                    filteredUsers.map(u => {
                                        ...
                                        return (
                                            <tr key={u.id} className="hover:bg-slate-50 transition-colors bg-white">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${roleColor}`}>
                                                        {u.role}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-xs text-slate-500">
                                                    {u.role === 'vet' && u.clinic_name && <span className="block font-semibold text-slate-700">Clinic: {u.clinic_name}</span>}
                                                    ...
                                                    {!isProvider && <span className="italic">N/A</span>}
                                                </td>
                                                <td className="px-6 py-4">
                                                    {isProvider ? (
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    {isProvider && (
                                                        !isApproved ? (
                                                            <button onClick={() => handleVerify(u.id, true)} className="inline-flex items-center gap-1 px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold text-xs rounded-lg transition-colors border border-emerald-200">
                                                                <span className="material-symbols-outlined text-[16px]">check</span> Approve
                                                            </button>
                                                        ) : (
                                                            <button onClick={() => handleVerify(u.id, false)} className="inline-flex items-center gap-1 px-3 py-1.5 bg-slate-50 hover:bg-slate-100 text-slate-600 font-bold text-xs rounded-lg transition-colors border border-slate-200">
    ...
    const renderBookings = () => {
        ...
        return (
            <div className="animate-fade-in flex flex-col h-full">
                <h1 className="text-2xl font-bold text-slate-900 mb-6">Booking Ledger</h1>
